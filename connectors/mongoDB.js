...
const Student = require('../models/mongoDB/student');
const FeeCode = require('../models/mongoDB/feecode');
const Enrollment = require('../models/mongoDB/enrollment');
const MarksCode = require('../models/mongoDB/markscode');
...
const getUser_C = user => {
  ...
  return User.find({ _id: user.id }).then((res) => {
    ...
    if (res.length > 0) {
      ...
      return { name: res[0].name, email: res[0].email, phone: res[0].phone, skey: res[0].skey, message : "User not found, Please logout and login again." }
    } else {
      return { name: "", email: "", phone:"", skey:"", message : "User not found, Please logout and login again." };
    }
  });
};
...
const checkUserExists_C = input => {
  ...
  return User.find({ email: input.email }, { name: 1, email: 1,phone: 1,skey: 1, roles: 1 });
};
...
const updateUser_C = input => {
  ...
  return User.findByIdAndUpdate(input.id, input, function (err, res) {
    ...
    if (res) {
      return { name: res.name, email: res.email, phone: res.phone, skey: res.sky, message: "Success" };
    } else {
...
// student functions
const setStudent_C = input => {
  let student = new Student(input);
  return Student.find({ CODE: input.CODE }).then((res) => {
    if (res != "") {
      return { CODE: "", message: "Code Already Exists." };
    } else {
      student.save();
...
const setStudentDoc_C = input => {
  return Student.findByIdAndUpdate(input._id, input, function (err, res) {
    if (err) {
      console.log(err);
    }
    if (res) {
...
const delStudentDoc_C = input => {
return Student.findByIdAndRemove(input._id, function (err, res) {
  if (err) {
    console.log(err);
  }
  if (res) {
...
const getStudent_C = input => {
  return checkRoles(input).then((res) => {
    if (res.authenticated) {
      if (input._id) {
        // search OneDocByID
        var ObjectId = require('mongoose').Types.ObjectId;
        return Student.findOne({ _id: new ObjectId(input._id) }).then((res)=> {
          return [res];
        });
      } else {
        // search all docs
        return Student.find({ CODE: { "$regex": input.CODE, "$options": "i" }, DESCR: { "$regex": input.DESCR, "$options": "i" } }).then((res) => {
          if (res != "") {
            return res;
          } else {
            return [{ _id: "", CODE: "", DESCR: "", message: "Success" }];
...
// fee codes functions
const getFeeCode_C = input => {
  return checkRoles(input).then((res) => {
    if (res.authenticated) {
      if (input._id) {
        // search OneDocByID
        var ObjectId = require('mongoose').Types.ObjectId;
        return FeeCode.findOne({ _id: new ObjectId(input._id) }).then((res)=> {
          return [res];
        });
      } else {
        // search all docs
        return FeeCode.find({ code: { "$regex": input.code, "$options": "i" }, descr: { "$regex": input.descr, "$options": "i" } }).then((res) => {
          if (res != "") {
            return res;
          } else {
            return [{ _id: "", code: "", descr: "", message: "Success" }];
...
const setFeeCode_C = input => {
  let feecode = new FeeCode(input);
  return FeeCode.find({ code: input.code }).then((res) => {
    if (res != "") {
      return { code: "", message: "Code Already Exists." };
    } else {
      feecode.save();
...
const setFeeCodeDoc_C = input => {
  let feecode = new FeeCode(input);
  return FeeCode.findByIdAndUpdate(input._id, input, function (err, res) {
    if (err) {
      console.log(err);
    }
    if (res) {
...
const delFeeCodeDoc_C = input => {
return FeeCode.findByIdAndRemove(input._id, function (err, res) {
  if (err) {
    console.log(err);
  }
  if (res) {
...
// Marks codes functions
const setMarksCode_C = input => {
  let markscode = new MarksCode(input);
  
  return MarksCode.find({ code: input.code }).then((res) => {
    if (res != "") {
      return { code: "", message: "Code Already Exists." };
    } else {
      markscode.save();
...
const setMarksCodeDoc_C = input => {
  let markscode = new MarksCode(input);
  return MarksCode.findByIdAndUpdate(input._id, input, function (err, res) {
    if (err) {
      console.log(err);
    }
    if (res) {
...
const delMarksCodeDoc_C = input => {
return MarksCode.findByIdAndRemove(input._id, function (err, res) {
  if (err) {
    console.log(err);
  }
  if (res) {
...
const getMarksCode_C = input => {
  return checkRoles(input).then((res) => {
    if (res.authenticated) {
      if (input._id) {
        // search OneDocByID
        var ObjectId = require('mongoose').Types.ObjectId;
        return MarksCode.findOne({ _id: new ObjectId(input._id) }).then((res)=> {
          return [res];
        });
      } else {
        // search all docs
        return MarksCode.find({ code: { "$regex": input.code, "$options": "i" }, descr: { "$regex": input.descr, "$options": "i" } }).then((res) => {
          if (res != "") {
            return res;
          } else {
            return [{ _id: "", code: "", descr: "", message: "Success" }];
...
// Enrollment codes functions
const setEnrollment_C = input => {
  let enrollment = new Enrollment(input);
  return Enrollment.find({ code: input.code }).then((res) => {
    if (res != "") {
      return { code: "", message: "Code Already Exists." };
    } else {
      enrollment.save();
...
const setEnrollmentDoc_C = input => {
  let enrollment = new Enrollment(input);
  return Enrollment.findByIdAndUpdate(input._id, input, function (err, res) {
    if (err) {
      console.log(err);
    }
    if (res) {
...
const delEnrollmentDoc_C = input => {
return Enrollment.findByIdAndRemove(input._id, function (err, res) {
  if (err) {
    console.log(err);
  }
  if (res) {
...
const getEnrollment_C = input => {
  return checkRoles(input).then((res) => {
   if (res.authenticated) {
      if (input._id) {
        // search OneDocByID
        var ObjectId = require('mongoose').Types.ObjectId;
        return Enrollment.findOne({ _id: new ObjectId(input._id) }).then((res)=> {
          return [res];
        });
      } else {
        // search all docs
        return Enrollment.find({ code: { "$regex": input.code, "$options": "i" }, descr: { "$regex": input.descr, "$options": "i" } }).then((res) => {
          if (res != "") {
            return res;
          } else {
            return [{ _id: "", code: "", descr: "", message: "Success" }];
